"""Audit tests for trigonometry training and testing datasets."""
...
from trig_datasets import TRIG_TESTING_DATASET, TRIG_TOPICS, TRIG_TRAINING_DATASET
...
@pytest.mark.parametrize("topic", TRIG_TOPICS)
def test_training_topic_has_ten_problems(topic: str) -> None:
    items = [problem for problem in TRIG_TRAINING_DATASET if problem["topic"] == topic]
    assert len(items) == 10
...
def test_testing_topic_has_ten_problems(topic: str) -> None:
    items = [problem for problem in TRIG_TESTING_DATASET if problem["topic"] == topic]
    assert len(items) == 10
